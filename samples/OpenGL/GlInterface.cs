...
namespace OpenGL
{
    ...
    public unsafe class GlInterface : GlBasicInfoInterface<GlInterface.GlContextInfo>
    {
        ...
        public static GlInterface FromNativeUtf8GetProcAddress(GlVersion version, Func<string, IntPtr> getProcAddress) =>
        ...
        public void ShaderSourceString(int shader, string source)
        {
            ...
            {
                var len = new IntPtr(bytes.Length);
        ...
        public void BindAttribLocationString(int program, int index, string name)
        {
            fixed(byte* ptr = Encoding.UTF8.GetBytes(name))
                BindAttribLocation(program, index, (IntPtr)ptr);
        }
        ...
        {
            fixed(byte* ptr = Encoding.UTF8.GetBytes(name))
                return GetAttribLocation(program, (IntPtr)ptr);
        }
        ...
        {
            fixed(byte* ptr = Encoding.UTF8.GetBytes(name))
                return GetUniformLocation(program, (IntPtr)ptr);
        }
